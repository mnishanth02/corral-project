...
pnpm install
pnpm db:push                  # push the (empty) Drizzle schema — verifies the DB connection
...
|---|---|---|---|
| `api` | `apps/api/Dockerfile` | web | `DATABASE_URL=${{Postgres.DATABASE_URL}}`, `REDIS_URL=${{Redis.REDIS_URL}}`, `API_PORT=3000`, `CORS_ORIGINS=<web+console URLs>`, `NODE_ENV=production` |
...
Railway uses each service's `railway.json` healthcheck (`/health` for api/worker). Run the DB push once
against the managed Postgres (e.g. a one-off `railway run pnpm db:push`).
